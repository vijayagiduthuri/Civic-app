...
const Overview = () => {
  ...
  const workersData = [
    { name: "Suresh Patil", department: "Sewage & Drainage", resolvedCount: 1, status: "available" },
    { name: "Lakshmi Devi", department: "Public Toilet Maintenance", resolvedCount: 2, status: "busy" },
    { name: "Ravi Kumar", department: "Waste Enforcement", resolvedCount: 1, status: "available" },
    { name: "Manoj Yadav", department: "Street Cleaning", resolvedCount: 1, status: "available" },
    { name: "Pradeep Sharma", department: "Waste Management", resolvedCount: 1, status: "available" },
    { name: "Anita Reddy", department: "Public Health", resolvedCount: 1, status: "available" }
  ];
  ...
  const highPriorityIssues = issuesData.filter(issue => issue.priority === "High").length;
  const totalWorkers = workersData.length;
  const availableWorkers = workersData.filter(worker => worker.status === "available").length;
  const totalResolvedByWorkers = workersData.reduce((sum, worker) => sum + worker.resolvedCount, 0);
  const avgResolutionTime = 2.3; // days
  const communityEngagement = issuesData.reduce((sum, issue) => sum + issue.upvotes, 0);
  ...
  const categoryChartData = [
    { category: 'Waste Management', count: issuesData.filter(i => i.category === 'Waste Management').length },
    { category: 'Sewage & Drainage', count: issuesData.filter(i => i.category === 'Sewage & Drainage').length },
    { category: 'Public Toilets', count: issuesData.filter(i => i.category === 'Public Toilet Maintenance').length },
    { category: 'Street Cleaning', count: issuesData.filter(i => i.category === 'Street Cleaning').length },
    { category: 'Illegal Dumping', count: issuesData.filter(i => i.category === 'Illegal Waste Dumping').length },
    { category: 'Sewer System', count: issuesData.filter(i => i.category === 'Sewer System Maintenance').length }
  ];
  ...
  const priorityDistribution = [
    { name: 'High', value: issuesData.filter(i => i.priority === 'High').length, color: '#dc2626' },
    { name: 'Medium', value: issuesData.filter(i => i.priority === 'Medium').length, color: '#d97706' }, 
    { name: 'Low', value: issuesData.filter(i => i.priority === 'Low').length || 1, color: '#16a34a' }
  ];

  const MetricCard = ({ title, value, subtitle, icon: Icon, trend, trendValue, color = "sky" }) => (
    <div className={`bg-white rounded-xl shadow-lg p-6 border border-${color}-100 hover:shadow-xl transition-all duration-300 hover:scale-[1.02]`}>
  ...
  const ChartCard = ({ title, children, className = "" }) => (
    <div className={`bg-white rounded-xl shadow-lg p-6 border border-sky-100 ${className}`}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 to-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Sanitation Management Overview</h1>
          <p className="text-gray-600">Comprehensive dashboard for monitoring city sanitation operations</p>
        </div>
          ...
          <ChartCard title="Top Performing Workers">
            <div className="space-y-4">
              {topPerformersData.map((worker, index) => (
                <div key={worker.name} className="flex items-center justify-between p-4 bg-sky-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="font-semibold text-green-600">{worker.resolvedCount}</span>
                    </div>
          ...
          <ChartCard title="Community Engagement Metrics">
            <div className="space-y-6">
              <div className="text-center p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg">
                <div className="text-4xl font-bold text-purple-600 mb-2">{communityEngagement}</div>
                <p className="text-sm text-gray-600 mb-4">Total Community Upvotes</p>
